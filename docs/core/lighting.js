...
export class WebGLLightingObject extends WebGLRenderingObject {
    constructor(gl, intialPos, // TODO: w = 1.0 时为有限位置， w = 0.0 时为平行光源 
    ambient, // 环境光
    diffuse, // 漫反射
    specular, // 镜面反射
    distFactors = [1, 0, 0], // 距离项因子
    drawSphere = true) {
        super(gl);
...
Object.assign(Canvas.prototype, {
    bindLighting(option) {
        ...
        initialPos, ambient, diffuse, specular, distFactors, drawSphere } = Object.assign(option, {
            distFactors: MV.vec3(1.0, 0.0, 0.0),
            drawSphere: true
        }, option);
        const light = new WebGLLightingObject(this.gl, initialPos, ambient, diffuse, specular, distFactors, drawSphere);
        ...
        this.updatePipeline.push(c => {
            for (const obj of this.objectsToDraw) {
                let uniforms = {
                    ...
                    u_Shininess: obj.uniforms.u_Shininess || 1.0,
                ...
                for (const prop of ["u_Ambient", "u_Diffuse", "u_Specular"]) {
                    if (prop in obj.uniforms) {
                        uniforms[prop] = MV.mult(uniforms[prop], obj.uniforms[prop]);
                    }
                }
                // 如果对象有标准颜色，则光照起增强效果
                ...
                if (obj.uniforms.u_DiffuseMap !== undefined) {
                    // 强化环境光的效果
                    uniforms.u_Ambient = MV.scale(2.0, uniforms.u_Ambient);
    ...
    bindShadowObject(srcObj, lighting) {
