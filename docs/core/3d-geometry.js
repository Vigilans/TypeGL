...
// 创建一个平面的顶点
export function createPlaneVertices(width = 1, depth = 1, subdivisionsWidth = 1, subdivisionsDepth = 1) {
...
// 创建一个球体的顶点
export function createSphereVertices(radius, subdivisionsAxis, subdivisionsHeight, opt_startLatitudeInRadians = 0, opt_endLatitudeInRadians = Math.PI, opt_startLongitudeInRadians = 0, opt_endLongitudeInRadians = Math.PI * 2) {
...
export function bindDrawing3D(createVertices) {
    return function (color, mode, source, center, ...args) {
...
Object.assign(Canvas.prototype, {
    drawFigure3D(color, mode, source, center, attributes, uniforms) {
...
// 通过轮廓创建3D模型
export function lathePoints(points, scale = [1, 1, 1], // 放缩倍数
axis = [0, 1, 0], // 旋转轴
...
export function generateNormals(model, maxAngle) {
    const positions = model.position.data;
    const texCoords = model.texCoord.data;
...
function makeIndexedIndicesFn(model) {
...
function makeUnindexedIndicesFn(model) {
    let ndx = 0;
...
function makeIndiceIterator(model) {
    return model.indices
        ? makeIndexedIndicesFn(model)
        : makeUnindexedIndicesFn(model);
}
//# sourceMappingURL=3d-geometry.js.map