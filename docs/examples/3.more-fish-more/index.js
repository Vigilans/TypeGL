...
async function main() {
    ...
    let fish_aqua = fish_aqua_raw.models[0].fields;
    delete fish_aqua.indices;
    let fish_aqua_obj = c.newOrientedObject(source, [0, 0, 1], [0, 1, 0], c.gl.LINES, fish_aqua, { u_Color: [...normRgb([156, 43, 51]), 1] });
    ...
    let fish_kun_obj = c.newOrientedObject(source, [0, 0, -1], [0, 1, 0], c.gl.LINES, fish_kun, { u_Color: [1, 0.5, 0, 1] });
    ...
    c.updatePipeline.push((cv, time, deltaTime) => {
        ...
        // MV.inverse4(tMatrix),
        MV.inverse4(cstMatrix), MV.translate(x, y, 0), MV.rotateZ(deltaTheta), cstMatrix, tMatrix, MV.scalem(0.2, 0.2, 0.2), MV.rotateX(-Math.PI / 2)));
    });
